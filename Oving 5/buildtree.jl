...
function work_with_dna_string(root, dna_string)
	dna_chr = dna_string[1]
	if !haskey(root.children, dna_chr)
		push!(root.children, dna_chr => Node(Dict{Char,Node}(), 1))
	else
		root.children[dna_chr].count += 1
	end
	if dna_string[2:end] != ""
		work_with_dna_string(root.children[dna_chr], dna_string[2:end])
	end
end
...
function buildtree(dnasequences)
	...
	for dna_string in dnasequences
		work_with_dna_string(root, dna_string)
	end
    return root
end
